...
class Block_decoder(nn.Module):
    def __init__(self, in_channels, out_channels, att_heads, dpr, img_size):
        ...
        self.coatt = CASC(F_g=in_channels // 2, F_x=in_channels // 2)
...
class Block_encoder_with_skip(nn.Module):
    def __init__(self, in_channels, out_channels, att_heads, dpr, img_size):
        ...
        self.layer1 = nn.Sequential(
            nn.ReLU(),
            nn.Conv2d(3, in_channels, kernel_size=3, stride=1, padding="same"),
        )
        self.layer2 = nn.Sequential(
            nn.ReLU(),
...
class LGMA(nn.Module):
    def __init__(self,args):
        ...
        filters = [32, 64, 128, 256, 512, 256, 128, 64, 32, 16]
        # filters = [64, 128, 256, 512, 1024, 512, 256, 128, 64, 32]
        ...
        # self.attention = UpBlock_attention(filters[4],filters[3],nb_Conv=2)

        # model
        self.Conv1 = conv_block(3, 16)
        self.Maxpool1 = nn.AvgPool2d(2,2)
        # [N,1,img_size,img_size] => [N,filters[0],img_size // 2,img_size // 2]
        self.block_1 = Block_encoder_without_skip(16, filters[0], att_heads[0], dpr[0], self.img_size)
        ...
        self.ds9 = DS_out(filters[8],filters[9], self.img_size // 2)
        self.ds10 = DS_out(filters[9],2,self.img_size)

    def forward(self, x):

        # Multi-scale input
        scale_img_1 = self.scale_img(x)  # x shape[batch_size,channel(1),224,224]
        scale_img_2 = self.scale_img(scale_img_1)  # shape[batch,1,56,56]
        ...
        out10 = self.ds10(out9)

        # print('end:',out10.shape)

        return out10

if __name__ == '__main__':
    input1 = torch.randn(1, 3, 448, 448)
    # input2 = torch.randn(1,1024,7,7)
    backbone = LGMA()
    x = backbone(input1)
