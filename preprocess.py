...
outputImg_path = "../bata/trainImage_k1_1217"
outputMask_path = "../bata/trainMask_k1_1217"
    ...
    for index, file in enumerate(tqdm(os.listdir(ct_path))):
        ...
        if int(np.sum(mask_crop)) != 0:
            for n_slice in range(mask_crop.shape[0]):
                ...
                ctImageArray = np.zeros((ct_crop.shape[1], ct_crop.shape[2], 3), np.float64)
